...
Un'associazione puó essere pensata come l'=insieme= delle sue occorrenze (essendo un'insieme non possono esserci duplicati)
...
- in id. esterna é possibile solo attraverso associazioni a cui l'entitá da identificare partecipa con cardinalita' (1,1)
...
- Inizialmente
  ...
  + standardizzare la sintassi
    - per ENTITÁ rappresentiamo ATTRIBUTI
...
Si crea uno schema che non é piú uno schema concettuale in quanto si introducono considurazioni applicative
...
- questa intersezione di attributi é quella usata dal natural join per ricamporre $R$
...
- andrebbero aggiuente le d.f. perse come vincoli globali
  + molto costoso, all'aggiunta di una tupla andrebbe fatto il join e ricontrollata la condizione


*** Le Forme Normali
Sono ricette di buona progottazione volte alla minimizzazione delle ridondanze e anomalie
...
=def= Attributo Primo
Data una relazione $R(A)$ gli attrbuti $Y \subseteq A$ sono detti *attributi primi* se e solo se $Y \subseteq K$, dove $K$ é chiave di $R(A)$
...
Se una relazione é in BCNF allora é anche in 3NF, l'inverso non é necessariamente vero.
La 3NF puó avere anomalie di inserimento e cancellazione
...
**** Attributo estraneo
Un attributo in uno d.f. che é possibile rimuovere continuaundo ad avere un insieme di d.f. equivalente
