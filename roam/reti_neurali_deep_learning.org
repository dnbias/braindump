...
Inoltre a ogni neurone $i$ è assegnato un vicinato (/neighberhood/) cui e' propagata la correzione dei pesi.
...
In questo caso il neurone è a conoscenza del suo errore e può andare a modificare gli input dei neuroni del livello successivo.

\[-\frac{\delta E}{\delta v_{j}} = - \frac{\delta E}{\delta e_{j}}\frac{\delta e_{j}}{\delta y_{j}}\frac{\delta y_{j}}{\delta v_{j}} = - e_{j}(-1)\phi' (v_{j})\]

\[\Delta w_{ji} = \eta e_{j}(-1) \phi' (v_{j})y_{i}\]
