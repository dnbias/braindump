...
- =vincolo=, una condizione
  + e' soddisfatto con una dato =assegnamento= che per essere una soluzione deve essere
...
2. controlla se e' una soluzione
3. se si =return= altrimenti =continue=

E' estremamente semplice ma non e' scalabile.

**** Profondita' con Backtracking
Si esplora l'albero delle possibili assegnazioni in profondita'. Si fa backtracking quando si incontra una assegnazione parziale che non seddisfa piu' le condizioni
Il problema e' che in =CSP= il ~branching factor~ e' spesso molto alto, producendo alberi molto larghi.
...
- l'albero avra' $n! \cdot d^{n}$ foglie

Questo e' migliorabile con la tecnica del /fuoco/ su una singola variabile a agni livello dell'albero, questo in quanto i =CSP= godono della proprieta' commutativa rispetta all'ordine delle  variabili. Questo permette di rimuove il fattoriale nel numero di foglie.

Uno dei difetti di questo approccio e' il =Thrashing=, riconsiderando assegnamenti successivi che si sono gia' dimostrati fallimentari durante l'esplorazione.
