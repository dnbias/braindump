...
**** Attributi
- Sintetizzati
  Il suo valore dipende da quello di attributi dei figli ed eventualmente da altri atributi di se stesso
- Eredditati
...
e ogni attributo ereditato \(X_i.e\) la regola semantica che definisce il valore di \(X_i.e\) dipende solo da
...
- grammatica in cui le produzioni sono arricchite da frammenti di codice
  - azioni semantiche
    ...
    + simili alle regole semantiche degli SDD
      - specificano il calcolo degli attributi ma anche codivce arbitrario
...
<<<<<<< HEAD
- il loro valore include sempre la concatenazione dei valori dello stesso attributo per tutte le variabili nel corpi di ogni produzione oltre che evetuali variabili ausiliarie
...
=======
- il loro valore include sempre la concatenzazione dei valori dello stesso attributo per tutte le variabili nel corpi di ogni produzione oltre che evetuali variabili ausiliarie
- la concatenzazione rispetta l'ordine delle variabili nel corpo delle produzioni
  Es, trasformazione da forma infissa a postfissa
\(E \to E_1 + T \{ E.post = E_1.post || T.post || "+" \}\)
...
2. considerando ogni variabile nell'ordine imposto si elimina laricorsione immediata per quella variabile e si riscrivono le occorrenze di quella variabile che compaiono nei corpi delle produzione delle variabili seguenti
* Linguaggi
** Linguaggio regolare
...
* JVM
Vedi: [[file:20200531062306-ijvm.org][IJVM]]
[[https://en.wikipedia.org/wiki/Java_bytecode_instruction_listings][Bytecode Instruction Listing]]
- Interprete bytecode
...
- non sono valutate espresioni booleane anche se banali: il problema é indecidibile
...
Il piú piccolo numero di slot necesari all'interno di un frame per la memorizzazione di argomenti e variabili locali
